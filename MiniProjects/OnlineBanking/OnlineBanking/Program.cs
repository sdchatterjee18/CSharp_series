        ...
        static void Main(string[] args)
        {
            ...
            switch (Choose_section)
            {
                    ...
                    switch (Choose_Operation)
                    {
                        ...
                        case 2:
                            ...
                            if (C_ID != 0)
                            {
                                ...
                                Console.WriteLine("\nWELCOME!! {0}", Name);
                                int? accountNo = account.GetAccountNoForTransactions(C_ID);
                                if (accountNo  != null)
                                {
                                    int AccountNo =Convert.ToInt32(accountNo);
                                    bool Check = true;
                                    ...
                                    while (Check)
                                    {
                                        ...
                                        {
                                            case 1:
                                                if (RetryPin(AccountNo) != 0)
                                                {
                                                    double? balance = account.GetBalanceByAccountNo(AccountNo);
                                                    if (balance != null)
                                                    {
                                                        double Balance = Convert.ToDouble(balance);
                                                        double? deposit = account.DepositAmountValidate();
                                                        if (deposit != null)
                                                        {
                                                            double Deposit = Convert.ToDouble(deposit);
                                                            balance = account.DepositAmountUsingAccountNo(AccountNo, Balance, Deposit);
                                                            if (balance != null)
                                                            {
                                            ...
                                            case 2:
                                                if (RetryPin(AccountNo) != 0)
                                                {
                                                    ...
                                                    {
                                                        double Balance =Convert.ToDouble(balance);
                                                        double? withdrawAmount = account.WithdrawAmountValidate(Balance);
                                                        if (withdrawAmount!= null)
                                                        {
                                                            double WithdrawAmount = Convert.ToDouble(withdrawAmount);
                                                            balance =account.WithdrawAmountUsingAccountNo(AccountNo, Balance, WithdrawAmount);
                                                            if (balance != null)
                                                            {
                ...
                case 2:
                    ...
                    while (check)
                    {
                        Console.WriteLine("1.Show All Customers details ->");
                        Console.WriteLine("CHOOSE :");
                        Choose_Operation = Convert.ToInt32(Authentication.CheckDigit());
                        switch (Choose_Operation)
                        {
                            case 1:
                                List<Admin> AllCustomerDetails =admin.GetAllCustomerDetails() ;
                                if (AllCustomerDetails != null)
                                {
                                    
                                    foreach (Admin Customers in AllCustomerDetails)
                                    {
                                        admin.DisplayDetailsOfEachCustomer(Customers);
                                    }
                                }

                                break;
                            default:
                                Console.WriteLine("you choosed wromg option!!");
                                break;
                        }
                        bool CheckAgin=true;
                        Console.WriteLine("Do you want to try Again ?");
                        ...
                        while (CheckAgin)
                        {
                            int Choose_again = Convert.ToInt32(Authentication.CheckDigit());
                            switch (Choose_again)
                            {
                                case 1:
                                    CheckAgin = false;
                    ...
                    break;
                default:
                    //while choosed wrong in admin and customer
                    break;
            }
            Console.ReadLine();
        }
